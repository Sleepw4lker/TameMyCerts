...
using CERTCLILib;
using CERTPOLICYLib;
using TameMyCerts.AdcsModules.PolicyModule.ClassExtensions;
...
namespace TameMyCerts.AdcsModules.PolicyModule;
...
public class Policy : ICertPolicy2
{
    ...
    {
        // TODO: Decide if we want to return the app name or a static string to hide the fact that this is a custom policy module.
        // It seems not possible to decide this based on a setting here as it seems the module is not initialized at this stage.
        // return _appName;
    ...
    public int VerifyRequest(string strConfig, int context, int isNewRequest, int flags)
    {
        ...
        var requestId = serverPolicy.GetRequestId();
        int disposition;
        ...
        var dbRow = serverPolicy.GetDatabaseRow();
    ...
    public void ShutDown()
    {
        ...
        {
            if (_windowsDefaultPolicyModule is not null)
            {
                Marshal.ReleaseComObject(_windowsDefaultPolicyModule);
                _windowsDefaultPolicyModule = null;
            }
        }
    }
    ...
    private void InitializeWindowsDefaultPolicyModule(string strConfig)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (_windowsDefaultPolicyModule != null)
            {
                Marshal.ReleaseComObject(_windowsDefaultPolicyModule);
            }
