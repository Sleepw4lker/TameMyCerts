...
{
    public static int GetRequestId(this CCertServerPolicy serverPolicy)
    {
        return serverPolicy.GetLongRequestPropertyOrDefault("RequestId");
    }

    public static CertificateDatabaseRow GetDatabaseRow(this CCertServerPolicy serverPolicy)
    {
        var notBefore = serverPolicy.GetDateCertificatePropertyOrDefault("NotBefore");
        var notAfter = serverPolicy.GetDateCertificatePropertyOrDefault("NotAfter");
        var keyLength = serverPolicy.GetLongCertificatePropertyOrDefault("PublicKeyLength");
        var publicKey = serverPolicy.GetBinaryCertificatePropertyOrDefault("RawPublicKey");
        var rawRequest = serverPolicy.GetBinaryCertificatePropertyOrDefault("RawRequest");
        var requestId = serverPolicy.GetLongRequestPropertyOrDefault("RequestID");
        var requesterName = serverPolicy.GetStringRequestPropertyOrDefault("RequesterName");
        var requestType = serverPolicy.GetLongRequestPropertyOrDefault("RequestType") ^ CertCli.CR_IN_FULLRESPONSE;
        var upn = serverPolicy.GetStringCertificatePropertyOrDefault("UPN") ?? string.Empty;
        var distinguishedName =
            serverPolicy.GetStringRequestPropertyOrDefault("Request.DistinguishedName") ?? string.Empty;
        var certificateTemplate = serverPolicy.GetStringCertificatePropertyOrDefault("CertificateTemplate");
        var requestAttributes = serverPolicy.GetRequestAttributes();
        var certificateExtensions = serverPolicy.GetCertificateExtensions();
        var keyAlgorithm =
            GetKeyAlgorithmFamily(serverPolicy.GetStringCertificatePropertyOrDefault("PublicKeyAlgorithm"));
        var subjectRelativeDistinguishedNames = serverPolicy.GetSubjectRelativeDistinguishedNames();
        ...
        this CCertServerPolicy serverPolicy)
    {
        ...
        {
            var value = serverPolicy.GetStringRequestPropertyOrDefault(RdnTypes.NameProperty[rdnType]);
    ...
    public static Dictionary<string, byte[]> GetCertificateExtensions(this CCertServerPolicy serverPolicy)
    {
        ...
        do
        {
            ...
            {
                extensionList.Add(extensionOid, serverPolicy.GetCertificateExtensionOrDefault(extensionOid));
            }
    ...
    public static void SetCertificateExtension(this CCertServerPolicy serverPolicy, string oid, byte[] value,
        bool critical = false)
    {
        // Kudos to Vadims Podans for his research and support!

        var pBstr = Marshal.AllocHGlobal(value.Length + 4);
        Marshal.WriteInt32(pBstr, 0, value.Length);
