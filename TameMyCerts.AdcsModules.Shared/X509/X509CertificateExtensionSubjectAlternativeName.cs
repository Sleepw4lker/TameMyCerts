...
namespace TameMyCerts.AdcsModules.Shared.X509;
...
public class X509CertificateExtensionSubjectAlternativeName : X509CertificateExtension
{
    ...
    /// <summary>
    ///     This is the actual constructor we use to parse the CSR. It may seem counter-intuitive to use a string here, this is
    ///     due to the fact that Interop does not work well with byte arrays.
    /// </summary>
    ...
    private void InitializeDecode(string rawData)
    {
        ...
        try
        {
            ...
            foreach (IAlternativeName san in extensionAlternativeNames.AlternativeNames)
            {
                ...
                ReleaseComObject(san);
            }
        }
        finally
        {
            ReleaseComObject(extensionAlternativeNames);
        }
    }
    ...
    public void InitializeEncode()
    {
        ...
        foreach (var keyValuePair in AlternativeNames)
        {
            ...
            ReleaseComObject(alternativeName);
        }

        var extensionAlternativeNames = new CX509ExtensionAlternativeNames();

        extensionAlternativeNames.InitializeEncode(alternativeNames);
        ReleaseComObject(alternativeNames);
    ...
    public void AddAlternativeName(string type, string value, bool throwOnError = false)
    {
        ...
        {
            throw new ArgumentException(string.Format("Unable to add entry of type \"{{0}}\" with value \"{{1}}\" to the Subject Alternative Name certificate extension. Either type or value is invalid.", type, value));
        }
    }
