...
public class DirectoryServiceValidatorTests
{
    ...
    private readonly DirectoryServiceValidator _validator = new(true);
    private readonly ITestOutputHelper _output;


    public DirectoryServiceValidatorTests(ITestOutputHelper output)
    {
        _output = output;
        // 2048 Bit RSA Key
    ...
    public void Allow_and_add_SID_extension_if_configured()
    {
        ...
        Assert.False(result.DeniedForIssuance);
        Assert.True(result.StatusCode.Equals(WinError.ERROR_SUCCESS));
        Assert.True(result.CertificateExtensions.ContainsKey(WinCrypt.szOID_NTDS_CA_SECURITY_EXT) &&
    ...
    public void Allow_and_add_SID_Uri_if_configured()
    {
        ...
        Assert.False(result.DeniedForIssuance);
        Assert.True(result.StatusCode.Equals(WinError.ERROR_SUCCESS));
        Assert.True(result.CertificateExtensions.ContainsKey(WinCrypt.szOID_SUBJECT_ALT_NAME2) &&
                    Convert.ToBase64String(
    ...
    public void Allow_but_dont_add_SID_extension_if_allowed()
    {
        ...
        Assert.False(result.DeniedForIssuance);
        Assert.False(result.CertificateExtensions.ContainsKey(WinCrypt.szOID_NTDS_CA_SECURITY_EXT));
    }
